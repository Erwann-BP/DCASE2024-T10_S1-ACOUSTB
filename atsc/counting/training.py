...
@hydra.main(version_base=None, config_path="../configs", config_name="leveraged_model")
def train(config: DictConfig) -> Trainer:
    ...
    if training_config.get("pretrained_model", None) is not None:
        ...
        # state_dict["regression.0.weight"] = torch.cat((state_dict["regression.0.weight"], torch.Tensor([[0,0],[0,0],[0,0],[0,0]])), dim=1)
