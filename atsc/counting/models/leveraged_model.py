...
"""BoostedBaseline neural network: Baseline model for traffic counting, leveraged with the estimations of an external counting algorithm."""
...
class BoostedBaseline(LightningModule):
    """Baseline model for traffic counting, leveraged with the estimations of an external counting algorithm."""
        ...
        *,
        ...
        return torch.nn.Sequential(
            *[
            ...
            BoostedBaseline.ConvToTime(mode=conv2time_mode),
            *[
                BoostedBaseline.LinearBlock(
                    in_features=in_features,
                    out_features=out_features,
                )
    ...
    def __init__(
        ...
        *,
        ...
        final_layer_units = 32,
        optimizer: type[torch.optim.Optimizer] = torch.optim.Adam,
    ):
        """
        Initialize the boosted baseline model: same architecture as the baseline model.
        ...
        self.logmel_extractor = BoostedBaseline._build_branch(
        ...
        self.gcc_extractor = BoostedBaseline._build_branch(
        ...
        self.merger_td = torch.nn.Sequential(
            *[
                BoostedBaseline.LinearBlock(in_features=in_features, out_features=out_features)
                for in_features, out_features in zip(
    ...
    def forward(self, audio: Tensor, estim_left: Tensor, estim_right: Tensor, n_files_train_dataset: Tensor) -> dict[str, Tensor]:
        """
        Forward pass of the boosted baseline model.

        Args:
            audio: (batch_size, n_channels, n_samples) raw waveform
            estim_left: (batch_size, 1) External counting estimation of vehicles in direction 'left'.
            estim_right: (batch_size, 1) External counting estimation of vehicles in direction 'right'.
            n_files_train_dataset: (batch_size, 1) Amount of real segments used to train or finetune the model. 
        ...
        # Definition of a coefficient which decreases as training dataset gets bigger
        coef_dataset_size = 1-min(n_files_train_dataset[0]/1440,1)
        
        sum_right = torch.sum(count_out[:,(0,2)], 1)
        sum_left = torch.sum(count_out[:,(1,3)], 1)
        
        # The defined coefficient acts as a weighting factor between the initial model output and the information brought by the external counting algorithm. 
        # Output (final_count_out_r): (batch_size, 2) real
        final_count_out_r = count_out[:,(0,2)]/torch.swapaxes(torch.cat((sum_right, sum_right)).view(2,batch_size),0,1) * ((coef_dataset_size * torch.swapaxes(torch.cat((estim_right, estim_right)).view(2,batch_size),0,1)) + (1-coef_dataset_size)*torch.swapaxes(torch.cat((sum_right, sum_right)).view(2,batch_size),0,1))
        # Output (final_count_out_r): (batch_size, 2) real
        final_count_out_l = count_out[:,(1,3)]/torch.swapaxes(torch.cat((sum_left, sum_left)).view(2,batch_size),0,1) * ((coef_dataset_size * torch.swapaxes(torch.cat((estim_left, estim_left)).view(2,batch_size),0,1)) + (1-coef_dataset_size)*torch.swapaxes(torch.cat((sum_left, sum_left)).view(2,batch_size),0,1))

        return dict(
            car_right=final_count_out_r[:, 0],
            car_left=final_count_out_l[:, 0],
            cv_right=final_count_out_r[:, 1],
            cv_left=final_count_out_l[:, 1],
        )
    ...
    def training_step(self, batch: dict[str, Tensor], batch_idx: int) -> Tensor:
        """
        Training step for the boosted baseline model.
        ...
        """
        output = self(batch["audio"], batch["estim_left"], batch["estim_right"], batch["n_files_train_dataset"])
    ...
    def validation_step(self, batch: dict[str, Tensor], batch_idx: int) -> None:
        """
        Validation step for the boosted baseline model.
    ...
    def predict_step(self, batch: dict[str, Tensor], batch_idx: int) -> dict[str, Tensor]:
        """
        Prediction step for the boosted baseline model.
    ...
    def configure_optimizers(self) -> torch.optim.Optimizer:
        """Configure the optimizer for the boosted baseline model."""
