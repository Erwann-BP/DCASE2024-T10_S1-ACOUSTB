...
from atsc.counting import gcc_count_estimation 
...
class TrafficCountDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        ### Boolean to choose whether to use the estimation processing method or not.
        self._estim = estim
        self._site = site
        ...
        if self._estim:
            for item in _index_dict:
                path = item["path"]
                audio, _ = librosa.load(self._root.joinpath(path), sr=self._sample_rate, mono=False)
    
                item["estim_left"], item["estim_right"] = gcc_count_estimation.full_evaluation_corr(Tensor(audio), self._site, self._root)
    ...
    def __getitem__(self, item: int) -> dict[str, Tensor | int]:
        ...
        segment["n_files_train_dataset"] = len(os.listdir(self._root.joinpath("train")))
...
class SyntheticTrafficDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> dict[str, Tensor | int]:
        ...
        segment = {
            ...
            "vehic_left": sum([1 for event in events_in_segment if event["direction"] == "left"]),
            "vehic_right": sum([1 for event in events_in_segment if event["direction"] == "right"]),
            "audio": audio_segment,
...
def _load_real_events_indices(
    indiv_events_root: str | Path,
) -> tuple[dict[str, dict[str, pd.DataFrame]], dict[str, dict[str, Path]]]:
    """
    Load pass-by indices for the given site.

    Args:
        indiv_events_root: Path to the root folder of the simulated pass-by events for the specific site.
    ...
    for vehicle_type in ["car", "cv"]:
        ...
        source_folder_map[vehicle_type] = {}
        for direction in ["left", "right"]:
            source_folder = Path(indiv_events_root) / vehicle_type / direction
            source_folder_map[vehicle_type][direction] = source_folder
            source_files = list(sorted(source_folder.glob("events-*.csv")))
            if len(source_files) == 0:
                raise ValueError(
...
class ConcatTrafficDataset(Dataset):
    """Traffic dataset, obtained through concatenation of individual passbys from other contexts."""
    ...
    def __init__(
        ...
        *,
        ...
        sample_rate: int = 16000,
        random: bool,
        indiv_events_root: Path | str,
        speed_tolerance: float = 10.0,
    ):
        """
        Initialize the concatenated traffic dataset.
        ...
        Args:
            ...
            sample_rate: Sample rate of the audio segment
            random: Set to randomly extract segments
            indiv_events_root: Path to the root folder of the real pass-by events for the specific site
            speed_tolerance: Tolerance for speed matching in the source events [km/h]
        ...
        # Load source events indices
        self._source_df_map, self._source_folder_map = _load_real_events_indices(indiv_events_root=indiv_events_root)
    ...
    def __getitem__(self, item: int) -> dict[str, Tensor | int]:
        ...
        for event in events_in_segment:
            ...
            event_path2 = event_path.__str__()[:-6] + str((int(event_path.__str__()[-6:-4])+22)%44) + ".wav"
            if event_path.__str__()[-11:-9] == "ME":
                event_path2 = event_path2.replace("_ME_","_SE_")
            else:
                event_path2 = event_path2.replace("_SE_","_ME_")
            event_path2 = Path(event_path2)
            event_paths = sorted([event_path, event_path2])
            
            event_audio12, _ = librosa.load(str(event_paths[0]), sr=48000, mono=False)
            event_audio34, _ = librosa.load(str(event_paths[1]), sr=48000, mono=False)
            
            event_audio = resample(x=np.concatenate((event_audio12, event_audio34), axis=0), num=int(self._event_duration.total_seconds() * self._sample_rate), axis=1)
